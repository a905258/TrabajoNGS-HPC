...
![**Figure 0:** Bladder Cancer.](images/BladderCancer.png){width="489"}
...
In this part, we are going to explain in detail the pipeline that has been followed to perform the RNA-Seq analysis. Figure 26 below shows a brief summary of the process.
...
In this step all FASTQC files from patients in the study were aligned against the reference human transcriptome. The objective here is to obtain a *.bam* file from each aligment.Here the code used for *Kallisto* [@bray2016near] aligment is shown:
...
This code was run in ATLAS PC in order to paralelize the job, at first 1 node and 16G of RAM were used but the job efficiency was 23.12%. In order to have a better usage of ATLAS resources, the job was sent again but only using 4G of RAM memory. Finally the efficiency reached has been represented in Figure 27.

<center>

![**Figure 27.** Efficiency of the process to do Kallisto.](images/seff%20Kallisto.png){width="511" height="240"}
...
![**Figure 28.** Gene Transcript Location Sample.](./images/GeneTranscriptLocation.png){width="650"}
...
![**Figure 29.** Part of the RNASeq2 dataframe.](./images/muestra_de_RNAseq2.png){width="650"}
...
![**Figure 30.** Design matrix.](./images/modelmatrix.png)
...
#the qvalue has been lowered from 0.05 to 0.001 because with 0.05 there were 45000 significant transcripts, with qval<=0.001 , 8198 transcripts  were kept.

```

<center>

![**Figure 32.** Sleuth significant matrix.](./images/sleuth_significant.png){width="700"}

</center>

However, as this method does not report the fold change, we also need to run the Wald test (WT), even though the former test is considered a better one. For most of the analyses and plots that will be carried out in this project the output corresponding to the LRT test will be used, employing the table generated with the WT test just to visualize the volcano plots.
...
![**Figure 33.** Sleuth Wald matrix.](./images/sleuth_wt.png){width="700"}
...
The enrichGO function of the ClusterProfiler R package was used to perform the Gene Ontology (GO) enrichment analysis for biological processes [@yu2012clusterprofiler], before this an extraction of the significant genes from our significant transcripts was performed.
...
#Extract all the genes (universe) from GeneTranscriptLocation: 67,128 genes from the 258710 anotated transcripts
universe <- unique(GeneTranscriptLocation$ensembl_gene_id)
```

Then gene ontology was performed with the *enrichGO* function. The GO terms obtained represent in the data frame how many of our significant genes perform a specific function. It is seen in the gene ratio that instead of studying 8031 genes there are only 4656, which are the codifying genes of the list of significant genes. As an example we can see that from the universe (20967 codifying genes) there are 165 genes that have cytoplasmic traslation associated, therefore from our 4656 genes we would expect that 36 actually have that funcion associated but actually there are 89, therefore this function is enriched.
...
![**Figure 34.** EnrichGO database.](./images/database_enrichGO.png){width="700"}
...
![**Figure 35.** EnrichGO simplified database.](./images/database_enrichGO_simplified.png){width="700"}

</center>

Based on the FDR value, 20 GO terms that have more of our significant genes associated, this is represented in a dotplot. In the gene ratio the % of significant genes/total number significant genes that have a specific function can be seen. The count represents the number of significant genes for that have that function.
...
![**Figure 36.** Dotplot.](./images/Dotplot.png){width="700"}
...
We searched manually for the chromosome segregation function in the *enrichGOsimplified* database.

<center>

![**Figure 37.** Chromosome segregation function.](./images/GO_chromosome_seggregation.png)
...
![**Figure 38.** Genes related to the chromosome seggregation.](./images/genes_chromosome_seggregation.png){width="233"}
...
Additionally, we also wanted to compare some of the genes that were mentioned in the paper with our list of statistically significant genes. On the one hand, the used gene list corresponded to the ones that were previously well-known bladder cancer mutated genes. On the other, we have taken into account some chromatin-remodeling genes where frequent mutations were found. In both cases, as those genes have been discovered performing mutation analyses in bladder cancer, it is expected that when compared with the significant genes obtained in our differential gene expression study, they do not coincide.
...
![**Figure 39.** IGV showing TACC3 and FGFR3.](./images/IGV.png)
...
Then, the following sbatch has been executed to perform the STAR alignment.
...
cd /scratch/a905208/FinalWork/STAR_new
 
STAR --runThreadN 3 --genomeDir /scratch/a905208/FinalWork/STAR/index --readFilesIn /scratch/a905208/FinalWork/fastq/SRR645259_1.fastq /scratch/a905208/FinalWork/faºstq/SRR645259_2.fastq --outFileNamePrefix S1_test --outSAMtype BAM SortedByCoordinate --outSAMunmapped Within --outSAMattributes NH HI AS NM MD
```

The seff of STAR was:

<center>

![**Figure 40.** Seff Star.](./images/Seff_STAR.png){width="700"}
...
![**Figure 41.** IGV visualization in STAR.](./images/IGV_STAR.png){width="700"}
...
![**Figure 42.** PCA plot.](./images/PCAplot.png){width="650"}
...
![**Figure 43.** Heatmap.](./images/Heatmap.png){width="700"}
...
![**Figure 44.** Boxplot data frame.](./images/data_boxplot.png){width="600"}
...
![**Figure 45.** Boxplot.](./images/Boxplot.png){width="700"}
...
![**Figure 46.** Volcano plot.](./images/Volcano%20plot.png)

</center>

## **5. CONCLUSIONS**

In conclusion, it can be said that this study has validated some of the results that were obtained by the researchers who published the scientific article that this project was based on. 

Regarding to DNA, the several space problems made it impossible to carry out all the steps required for the analysis, but the steps that were carried out were found to be correct. On the other hand, concerning the mutations, as only 8 samples were used, it is consistent that the results do not coincide with the scientific article. However, given this situation, an exhaustive analysis was carried out to discover the reasons why the results were so different, so it has been concluded that the DNA analysis was specific and correct. 

When it comes to the RNA-Seq study, it can be seen that the use of both Kallisto and Sleuth has provided a good approach for identifying differentially expressed transcripts between normal and malignant samples. In fact, the visualization tools that have been used throughout this section show not only that the obtained statistically significant transcripts can successfully separate the samples from the studied groups (PCA plot and Heatmap) but also that the difference in expression is indeed considerable (Boxplot and Volcano plot).
