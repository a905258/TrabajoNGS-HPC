...
Having obtained the output from Kallisto (the abundance.tsv, abundance.5 and .json for each sample), we have used the abundance.tsv files to make a dataframe containing the TPM values of each transcript per sample.
...
sleuth_table_lrt<-load ("sleuth_table_lrt.RData")
sleuth_significant<-load ("sleuth_significant_lrt.RData")
sleuth_table_wt<-load ("sleuth_table_wt.RData")
...
Then gene ontology was performed with the enrichGO function. The genes related to the gene transcripts with FDR < 0.01 were selected. The gene universe consisted of the 67,128 genes that were identified in *Ensembl* [cite]. So finally in database_enrichGO we obtained ?
...
![**Figure 20.** PCA.](./images/Heatmap.png)
...
sig_transcripts_sample <- data.frame(
  tran_name_1 = sapply(strsplit(sleuth_significant$target_id[1:10],"\\."),function(X) return(X[1])),
  tran_name_2 = sapply(strsplit(sleuth_significant$target_id[1:10],"\\."),function(X) return(X[2]))
  )
...
RNASeq2_filt_ord_sample <- log2(1+RNASeq2_filt_ord[sig_transcripts_sample,]) #esta linea no va
...
```
aqui poner foto del data frame de data boxplot
...
Fig_boxplot
```
![**Figure 21.** Boxplot.](./images/Boxplot.png)
...
![**Figure 22.** Volcano plot.](./images/Volcano plot.png)
