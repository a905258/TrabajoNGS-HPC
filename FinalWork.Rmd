...
In this step all FASTQC files from patients in the study were alligned against the reference human transcriptome. The objective here is to obtain a .bam file from each alligment.Here the code used for kallisto aligment is shown: 
...
This code was run in ATLAS PC in order to paralelize the job.at first 1 node and 16G of RAM were used but the job efficiency was 23.12%. In order to have a better usage of ATLAS resources, the job was sent again but only using 4G of RAM memory. Finally the efficiency reached 95.82% for CPU efficiency and 92.44% for Memory efficiency.

## Load and process the data generated with Kallisto
